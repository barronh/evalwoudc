...
parser.add_argument(
    '--season', default='ALL', choices=list(seasons),
...
if args.showbias:
    for tti, (ty, ymfb, ynmb) in enumerate(zip(sy, mfb, nmb)):
        if (tti % 5 == 0) or ty < 500:
            plt.text(5e3, ty, '{0:.0f}, {1:.0f}'.format(ynmb, ymfb))
...
if hasattr(sf, 'Station'):
    titlestr = '{} ({}, {}) ({}, {}, {})'.format(
        sf.Station, sf.Longitude, sf.Latitude, len(dates),
        min(dates).strftime('%Y-%m-%d'),
        max(dates).strftime('%Y-%m-%d')
    )
else:
    Station = sf.variables['PLATFORM_Name'][0, :]
    Station = Station.filled('').view('S64')[0].decode()
    Latitude = float(sf.variables['LOCATION_Latitude'][0])
    Longitude = float(sf.variables['LOCATION_Longitude'][0])
    titlestr = '{} ({}, {}) ({}, {}, {})'.format(
        Station, Longitude, Latitude, len(dates),
        min(dates).strftime('%Y-%m-%d'),
        max(dates).strftime('%Y-%m-%d')
    )
