...
parser.add_argument('--npoint', default = 10, type = int, help = 'Points for time smoothing')
...
n = args.npoint
def avgn(x):
    return np.ma.mean([np.ma.masked_values(x[i:-(n-i)], -999) for i in range(n)], axis = 0)
...
soax = fig.add_axes([.05, .2, .275, .7])
moax = fig.add_axes([.325, .2, .275, .7])
...
for ax in [moax, doax, soax]:
    ax.set_ylim(psrf, 50)
    ax.xaxis.set_major_locator(plt.matplotlib.dates.DayLocator([1]))
