...
for path in args.sondes:
    ...
    if mf is None:
        ...
        mf = mf.stack(modf, tk)
    sonf = pncopen(
        path, format='netcdf', addcf=False
    ).sliceDimensions(**{tk: ismine}).applyAlongDimensions(**{tk: 'mean'})
...
soax = fig.add_axes([.05, .1, .26, .8])
moax = fig.add_axes([.325, .1, .26, .8])
