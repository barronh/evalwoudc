...
parser.add_argument('--year', type = int, default = None, help = 'Restrict to a year, default any')
...
for prefix, opts in locations.items():
    ...
    dP_hPa = dP_Pa / 100
    """
    # Where are the NOAA data files; each sonde is in its own folder
    ...
    dates = dict([(datestr, strptime(datestr, '%Y-%m-%d %H:%M:%S%z')) for datestr in datestrs])
    
    # filter for correct month
    possibledatestrs = [datestr for datestr in datestrs if dates[datestr].month in months]
    
    if not args.year is None:
        possibledatestrs = [datestr for datestr in possibledatestrs if dates[datestr].year == args.year]

    mydatestrs = [datestr for datestr in possibledatestrs if os.path.exists(dates[datestr].strftime(args.intemplate))]
    notmydatestrs = [datestr for datestr in possibledatestrs if not os.path.exists(dates[datestr].strftime(args.intemplate))]
    ...
    modpaths = [dates[datestr].strftime(args.intemplate) for datestr in mydatestrs]
    ...
    for sondepath, modpath, modh in zip(sondepaths, modpaths, modhs):
        ...
        for dk in list(satmodf.dimensions):
            if dk not in ['LAY', 'STRLEN', 'ROW', 'COL', 'TSTEP', 'latitude', 'longitude', 'time', 'site', 'level', 'Pressure']:
